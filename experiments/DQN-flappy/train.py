...
def make_env(env_id, seed, capture_video, run_name, eval_mode=False, render_mode=None):
    ...
    env = gym.make(env_id, render_mode=render_mode, use_lidar=False)
...
def evaluate(model, device, run_name, num_eval_eps = 10, record = False, render_mode=None):
    ...
    for eps in tqdm(range(num_eval_eps)):
        ...
        while not done:
            ...
            # with torch.no_grad():
            action = model(torch.tensor(obs, device=device, dtype=torch.float32).unsqueeze(0)).argmax().item()
...
env = make_env(args.env_id, args.seed, args.capture_video, run_name, render_mode=None)
...
for step in tqdm(range(args.total_timesteps)):
    ...
    if rnd < eps:
        ...
        action = q_network(torch.tensor(obs, device=device, dtype=torch.float32).unsqueeze(0)).argmax().item()
    ...
    if step > args.learning_starts and step % args.train_frequency == 0:
        ...
        # with torch.no_grad():
        target_max = target_net(data.next_observations.to(torch.float32)).max(1)[0] # dim=1
        td_target = data.rewards.flatten() + args.gamma * target_max * (1 - data.dones.flatten())
        old_val = q_network(data.observations.to(torch.float32)).gather(1, data.actions).squeeze()
