"""Twin Delayed DDPG (TD3) with CNN networks for visual Gymnasium environments."""
...
def train_td3_cnn(
    ...
    """Train a CNN-based TD3 agent on pixel observations.
    ...
    if Config.n_envs > 1:
        ...
        obs_shape: Union[int, tuple[int, ...]] = (
            int(obs_space.n) if is_discrete_obs else tuple(obs_space.shape)  # type: ignore[attr-defined, arg-type]
        )
        action_shape = int(
            vec_env.single_action_space.n  # type: ignore[attr-defined]
            if isinstance(vec_env.single_action_space, gym.spaces.Discrete)
            else vec_env.single_action_space.shape[0]  # type: ignore[index]
        )
    else:
        # Single environment
        env_thunk = make_env(
    ...
    for step in tqdm(range(Config.total_timesteps)):
        ...
        new_obs, reward, terminated, truncated, info = env.step(action_np)  # type: ignore[union-attr]
