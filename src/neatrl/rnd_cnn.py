"""PPO with Random Network Distillation (RND) intrinsic exploration for visual Gymnasium environments."""
...
class Config:
    ...
    ext_gamma: float = (
        0.99  # Discount factor for extrinsic rewards (environment rewards)
    )
    int_gamma: float = 0.95  # Discount factor for intrinsic rewards (RND novelty)
...
class RunningMeanStd:
    ...
    def update(self, x: np.ndarray) -> None:
        """Update running statistics with a new batch of samples.

        Args:
            x (np.ndarray): Batch of new observations/rewards.
    ...
    def update_from_moments(
        ...
        """Update running statistics from pre-computed batch moments.

        Args:
            batch_mean (float | np.ndarray): Mean of the new batch.
            batch_var (float | np.ndarray): Variance of the new batch.
            batch_count (int): Number of samples in the new batch.
...
class CriticNet(nn.Module):
    """Extrinsic + intrinsic value critic for RND."""
...
class PredictorNet(nn.Module):
    """RND predictor network trained to mimic the frozen target."""
...
class TargetNet(nn.Module):
    """Frozen RND target network providing the novelty signal."""
...
def train_ppo_rnd_cnn(
    env_id: str = Config.env_id,  # type: ignore[assignment]
    env: Optional[gym.Env] = None,
    total_timesteps: int = Config.total_timesteps,
    seed: int = Config.seed,
    lr: float = Config.lr,
    ext_gamma: float = Config.ext_gamma,
    int_gamma: float = Config.int_gamma,
    ...
    EXT_COEFF: float = Config.EXT_COEFF,
    INT_COEFF: float = Config.INT_COEFF,
    ...
    predictor_class: Any = PredictorNet,
    target_class: Any = TargetNet,
    ...
    """Train a CNN PPO+RND agent on pixel observations with intrinsic exploration.

    Args:
    env_id (str): Gymnasium environment ID. Mutually exclusive with ``env``.
    ...
    ext_gamma (float): Discount factor for extrinsic (environment) rewards.
    int_gamma (float): Discount factor for intrinsic (RND novelty) rewards.
    ...
    EXT_COEFF (float): Weight on the extrinsic (environment) reward.
    INT_COEFF (float): Weight on the intrinsic (RND novelty) reward.
    ...
    predictor_class (type | nn.Module): Custom class or instance for the RND predictor network.
    target_class (type | nn.Module): Custom class or instance for the frozen RND target network.
    ...
    Config.seed = seed
    Config.lr = lr
    Config.ext_gamma = ext_gamma
    Config.int_gamma = int_gamma
    ...
    Config.EXT_COEFF = EXT_COEFF
    Config.INT_COEFF = INT_COEFF
    ...
    # Initialize running mean/std for intrinsic reward normalization
    intrinsic_reward_rms = RunningMeanStd(shape=())
    ...
    intrinsic_rewards_storage = torch.zeros((Config.max_steps, Config.n_envs)).to(
        device
    )
    dones_storage = torch.zeros((Config.max_steps, Config.n_envs)).to(device)
    ext_values_storage = torch.zeros((Config.max_steps, Config.n_envs)).to(device)
    int_values_storage = torch.zeros((Config.max_steps, Config.n_envs)).to(device)
    ...
    for update in tqdm(
                ...
                wandb.log(
                    {
                        ...
                        "rewards/intrinsic_reward_std": np.sqrt(
                            intrinsic_reward_rms.var
                        ),
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(Config.max_steps)):
                ...
                rt_ext = rewards_storage[t] + Config.ext_gamma * ext_gt_next_state

                rt_int = (
                    intrinsic_rewards_storage[t] + Config.int_gamma * int_gt_next_state
                )
        ...
        advantages = (Config.INT_COEFF * int_advantages) + (
            Config.EXT_COEFF * ext_advantages
        )
        if Config.use_wandb:
            wandb.log(
                {
        ...
        for _epoch in range(Config.PPO_EPOCHS):
            ...
            for start in range(0, batch_size, minibatch_size):
                ...
                rnd_loss = F.mse_loss(pred_features, target_features)
            ...
            wandb.log(
                {
                    ...
                    "losses/intrinsic_critic_loss": int_critic_loss.item(),
                    "losses/extrinsic_critic_loss": ext_critic_loss.item(),
