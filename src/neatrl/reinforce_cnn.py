"""REINFORCE policy gradient with CNN policy for visual Gymnasium environments."""
...
def train_reinforce_cnn(
    ...
    """Train a CNN-based REINFORCE agent on pixel observations.
    ...
    if Config.n_envs > 1:
        action_shape = (
            ...
            else env.single_action_space.shape  # type: ignore[attr-defined]
        )
    else:
        action_shape = (
            env.action_space.n
    ...
    if Config.custom_agent is not None:
        if isinstance(Config.custom_agent, nn.Module):
            ...
            validate_policy_network_dimensions(
                Config.custom_agent,
                obs_shape,  # type: ignore[arg-type]
                action_shape,  # type: ignore[arg-type]
            )
    ...
    dashboard = (
        Dashboard("REINFORCE-CNN", Config.env_id or "custom", Config.total_timesteps)
    ...
    for step in tqdm(range(updates)):
        ...
        while True:
            ...
            # Handle both discrete and continuous action spaces
            if Config.n_envs > 1:
                action_space = env.single_action_space  # type: ignore[attr-defined]
            ...
            if Config.use_entropy:
                entropies.append(dist.entropy())  # type: ignore[union-attr]
