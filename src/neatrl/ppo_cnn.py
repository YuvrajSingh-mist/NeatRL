"""Proximal Policy Optimization (PPO) with CNN networks for visual Gymnasium environments."""
...
def train_ppo_cnn(
    ...
    """Train a CNN-based PPO agent on pixel observations, returning the trained actor.
    ...
    dashboard = (
        Dashboard("PPO-CNN", Config.env_id or "custom", Config.total_timesteps)
        ...
        if Config.use_wandb and update % 10 == 0:
            ...
            if dashboard:
                dashboard.update(
                    ...
                    epoch=step if "step" in dir() else update,
                    losses={
