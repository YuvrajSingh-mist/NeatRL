...
def evaluate(model, device, run_name, num_eval_eps = 10, record = False):
    ...
    for eps in range(num_eval_eps):
        ...
        while not done:
            ...
            # with torch.no_grad():
            with torch.no_grad():
                action = model(torch.tensor(obs, device=device, dtype=torch.float32).unsqueeze(0))
...
for step in tqdm(range(args.total_timesteps)):
    ...
    if step > args.learning_starts:
        ...
        with torch.no_grad():
            ...
            target_max1 = target_q1_network(data.next_observations.to(torch.float32), next_actions)
            target_max2 = target_q2_network(data.next_observations.to(torch.float32), next_actions)
            target_max = torch.min(target_max1, target_max2)
            td_target = data.rewards + args.gamma * target_max * (1 - data.dones)

        old_val1 = q1_network(data.observations.to(torch.float32), data.actions.to(torch.float32))
        old_val2 = q2_network(data.observations.to(torch.float32), data.actions.to(torch.float32))
        q1_optim.zero_grad()
        ...
        if step % args.train_frequency == 0:
            ...
            action_values = -q1_network(data.observations.to(torch.float32), actions).mean()  # Maximize Q-value for next actions
